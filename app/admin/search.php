...
$subdomain=$_SESSION['subdomain'];
		...
		$results=mysqli_query($conn2,"SELECT* FROM $subdomain WHERE name LIKE '%$search%' OR  form LIKE '%$search%'  OR issuedate LIKE '%$search%' OR period LIKE '%$search%' OR destination LIKE '%$search%' OR reason LIKE '%$search%' OR session LIKE '%$search%'");
...
$firstCharacter = $string[0];
	echo
		'<tr>
                                                ...
                                                <td class="text-muted"><h7>'.$row["issuedate"].'</h7></br><i class="fa fa-map-marker"></i> '.$row["destination"].'    ( '.$row["house"].' house )</br>'.$row["period"].' day(s) <i class="fa fa-clock-o"></i> <small class="text-muted">     Issued by: <b>'.$row["author"].'</b></small></td>
                                            </tr>';
	}
			}
